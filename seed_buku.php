<?php
/**
 * Script untuk menambahkan 100+ buku ke database perpustakaan
 * Jalankan via browser: http://localhost/perpustakaan/seed_buku.php
 */
...
// Check apakah sudah pernah dijalankan
$stmt = $db->query("SELECT COUNT(*) as total FROM buku");
$existingCount = $stmt->fetch()->total;

echo "<html><head><title>Seed Buku - Perpustakaan</title>";
echo "<style>body{font-family:Arial,sans-serif;max-width:900px;margin:40px auto;padding:20px;}
h1{color:#6C3BAA;}table{border-collapse:collapse;width:100%;margin-top:20px;}
th,td{border:1px solid #ddd;padding:8px;text-align:left;font-size:13px;}
th{background:#6C3BAA;color:white;}tr:nth-child(even){background:#f9f9f9;}
.success{color:green;font-weight:bold;}.info{color:#6C3BAA;}</style></head><body>";

echo "<h1>📚 Seed Data Buku - Perpustakaan Digital</h1>";
echo "<p class='info'>Jumlah buku di database saat ini: <strong>{$existingCount}</strong></p>";

if ($existingCount > 10) {
    echo "<p class='info'>⚠️ Database sudah memiliki banyak data. Seed dibatalkan untuk menghindari duplikasi.</p>";
    echo "<p>Jika ingin menambahkan ulang, hapus data buku terlebih dahulu.</p>";
    echo "</body></html>";
    exit;
}
...
$stmt = $db->prepare("INSERT INTO buku (judul, pengarang, penerbit, tahun_terbit, isbn, jumlah_stok, kategori_id) VALUES (:judul, :pengarang, :penerbit, :tahun_terbit, :isbn, :jumlah_stok, :kategori_id)");

echo "<table><tr><th>#</th><th>Judul</th><th>Pengarang</th><th>Penerbit</th><th>Tahun</th><th>Status</th></tr>";
...
foreach ($books as $i => $book) {
    ...
    echo "<tr><td>" . ($i + 1) . "</td><td>{$book[0]}</td><td>{$book[1]}</td><td>{$book[2]}</td><td>{$book[3]}</td><td>{$status}</td></tr>";
}

echo "</table>";
echo "<br><p class='success'>✅ Selesai! {$inserted} buku berhasil ditambahkan. {$errors} gagal.</p>";

$stmt2 = $db->query("SELECT COUNT(*) as total FROM buku");
$newCount = $stmt2->fetch()->total;
echo "<p class='info'>Total buku di database sekarang: <strong>{$newCount}</strong></p>";
echo "<p><a href='" . App::BASE_URL . "/index.php?page=buku'>← Kembali ke Kelola Buku</a></p>";
echo "</body></html>";
